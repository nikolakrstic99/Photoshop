...
struct BMPFileHeader {
    uint16_t file_type{ 0x4D42 }; //def    // File type always BM which is 0x4D42 (stored as hex uint16_t in little endian)
    uint32_t file_size{ 0 };               // Size of the file (in bytes)
    uint16_t reserved1{ 0 };      //def         // Reserved, always 0
...
public:
    ...
    int getWidth() { return bmp_info_header.width; }
    ...
    int getDepth() { return bmp_info_header.bit_count / 8; }
